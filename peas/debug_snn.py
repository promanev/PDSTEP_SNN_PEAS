...
from peas.tasks.debug_task import DebugTask
...
if __name__ == '__main__':
    ...
    exp_id = 'disemb_'+str(n_nodes_input)+'i_'+str(n_nodes_hidden)+'h_'+str(n_nodes_output)+'o_'+'pop' \
             +str(popsize)+'_gen'+str(generations)+'_test_run5'
    ...
    substrate.add_nodes([(x, y, 2) for y in np.linspace(-1.0, 1.0, 1)
                                   for x in np.linspace(-1.0, 1.0, 5)], 'hidden')
