...
class NEATGenotype(object):
    ...
    def __init__(self, 
        ...
        if self.bias_as_node:
            print "WARNING: Added bias node to inputs"
            self.inputs += 1
        ...
        if topology is None:
            ...
            for i in xrange(self.inputs):
                ...
                #
                #: Tuples of (fforder, type, bias, response, layer)
            ...
            for i in xrange(self.inputs):
                for j in xrange(self.inputs, self.inputs + self.outputs):
                    ...
                    if innov==0:
                        ...
                        self.conn_genes[(i, j)] = [innov, i, j, 0, True]
                        print "Conn.gene #",innov,"From:",i,"To:",j,"Weight =",0.0
                    innov += 1
            # print "Bias_as_node:",self.bias_as_node
            # print "Number of inputs =",self.inputs
            # print "Number of outputs =",self.outputs
            # print "Node genes: \n", self.node_genes                    
            # print "Conn genes: \n", self.conn_genes
    ...
    def visualize_cppn(self, filename):
        return NeuralNetwork(self).visualize_cppn(filename, inputs=self.inputs, outputs=self.outputs, plot_bias=0)
